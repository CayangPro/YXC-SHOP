...
<div id="bannerFatherBox">
	...
	<ul class="bannerRight">
		<li class="bannerFirst">
			<a href="{{U('Index/index')}}" target="_blank" {{if value="$c  eq 'Index'"}} style="color: #FF335A;"{{endif}} >首页</a>
		</li>
		{{foreach from="$cateData" value="$v"}}
			<li>
				<a href="{{U('List/index',['cid'=>$v['cid']])}}" target="_blank" {{if value="$v['cid'] eq $_GET['cid']"}}style="color: #FF335A" {{endif}}>{{$v['cname']}}</a>
			</li>
		{{endforeach}}
