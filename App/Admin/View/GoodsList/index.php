		...
		<table class="table table-bordered">
			...
			{{foreach from="$listData" value="$v"}}
				<tr style="text-align:center">
					...
					<td>{{$v['glnumber']}}</td>					
					<td>						
						<a href="{{U('edit',array('glid'=>$v['glid'],'zol_goods_gid'=>$v['zol_goods_gid']))}}" class="btn btn-sm btn-warning">编辑</a>
						<a href="javascript:if(confirm('确定删除吗？')) location.href='{{U('del',array('glid'=>$v['glid']))}}';" class="btn btn-sm btn-danger" class="btn btn-sm btn-danger">删除</a>
					</td>
				</tr>
			{{endforeach}}
			<form action="{{U('GoodsList/add')}}" method="post">		
				<tr style="text-align:center">
					<th>添加货品</th>
					{{foreach from="$data" value="$v"}}
						<td>
							<select name="combineid[]" class="form-control input-sm">
								...
								{{foreach from="$v['value']" value="$value"}}
									<option value="{{$value['gaid']}}">{{$value['gavalue']}}</option>
								{{endforeach}}
							</select>
						</td>
					{{endforeach}}
